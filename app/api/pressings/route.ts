...
import { READ_ROLES, assertRole, getRequestId, resolveAuthenticatedActor, WRITE_ROLES } from '@/server/shared/request-context';
...
export async function DELETE(request: Request) {
  const requestId = getRequestId(request);
  const message = 'La suppression physique d’un apport est désactivée. Utilise l’annulation contrôlée.';

  logger.warn({
    action: 'pressings.delete.disabled',
    requestId,
    details: { message },
  });
  ...
  return NextResponse.json(
    ...
    { status: 405, headers: { Allow: 'GET, POST', 'x-request-id': requestId } },
  );
}
