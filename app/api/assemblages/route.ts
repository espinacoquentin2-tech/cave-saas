...
export async function POST(request: Request) {
  ...
  try {
    ...
    logger.info({
      action: 'assemblages.post.success',
      requestId,
    ...
    if (error instanceof ZodError) {
      logger.warn({
        action: 'assemblages.post.validation_failed',
        requestId,
    ...
    if (error instanceof BusinessLogicError) {
      logger.warn({
        action: 'assemblages.post.business_rejected',
        requestId,
        details: { message: error.message },
      });
    ...
    logger.error({
      action: 'assemblages.post.unhandled_error',
      requestId,
