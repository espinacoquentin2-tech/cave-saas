...
export async function DELETE(request: Request) {
  try {
    ...
    // On s'adapte automatiquement : si votre base utilise des chiffres (1, 2) ou du texte (uuid)
    const containerId = isNaN(Number(id)) ? id : Number(id);

    // Ordre de destruction à la base de données
    ...
    console.error("ERREUR SUPPRESSION CUVE :", error);
    // Renvoie la vraie erreur au cas où la cuve refuse de se supprimer
    return NextResponse.json({ error: error.message || "Impossible de supprimer la cuve" }, { status: 400 });
  }
}