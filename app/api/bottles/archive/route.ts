...
import { archiveBottleLotSchema } from '@/server/modules/bottles/bottle.schemas';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const payload = archiveBottleLotSchema.parse(await request.json());
    const result = await BottleModuleService.archive(payload, actor);

    logger.info({
      action: 'bottles.archive.post.success',
      requestId,
      userEmail: actor.email,
      role: actor.role,
      details: { bottleLotId: payload.bottleLotId },
    });

    return NextResponse.json({ status: 'SUCCESS', data: result }, { status: 201, headers: { 'x-request-id': requestId } });
  } catch (error) {
    ...
    if (error instanceof ZodError) {
      logger.warn({ action: 'bottles.archive.post.validation_failed', requestId, details: { issues: error.flatten() } });
      return NextResponse.json(
    ...
    if (error instanceof BusinessLogicError) {
      logger.warn({ action: 'bottles.archive.post.business_rejected', requestId, details: { message: error.message } });
      return NextResponse.json(
    ...
    logger.error({
      action: 'bottles.archive.post.unhandled_error',
      requestId,
