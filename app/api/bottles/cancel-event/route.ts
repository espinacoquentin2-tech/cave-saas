...
import { cancelBottleEventSchema } from '@/server/modules/bottles/bottle.schemas';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const payload = cancelBottleEventSchema.parse(await request.json());
    const result = await BottleModuleService.cancelEvent(payload, actor);

    logger.info({
      action: 'bottles.cancel-event.post.success',
      requestId,
    ...
    if (error instanceof ZodError) {
      logger.warn({ action: 'bottles.cancel-event.post.validation_failed', requestId, details: { issues: error.flatten() } });
      return NextResponse.json(
    ...
    if (error instanceof BusinessLogicError) {
      logger.warn({ action: 'bottles.cancel-event.post.business_rejected', requestId, details: { message: error.message } });
      return NextResponse.json(
    ...
    logger.error({
      action: 'bottles.cancel-event.post.unhandled_error',
      requestId,
